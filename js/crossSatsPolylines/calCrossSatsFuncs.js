...
function initializeCrossSatsTable1(crossSatsArray) {
    ...
    //注册选中jqx datatable中选中某一行触发的事件
    $("#crossSatsTable").on('rowSelect', function(event) {
        // event arguments
        //var args = event.args;
        // row data
        ...
        //选中某一行，过境的轨迹中相应轨迹高亮显示
        $.each(satTraces._polylines, function(i, val) {
            //判断点击的某一行与某条线匹配
            if (this.satNum == rowData.satNum) {
                var beginDate = Cesium.JulianDate.toDate(this.beginDateEpoch);
                var endDate = Cesium.JulianDate.toDate(this.endDateEpoch);
                if (getDatestringFromDate(beginDate) == rowData.beginDate && getDatestringFromDate(endDate) == rowData.endDate) {
                    satTraceMaterial = this._material;
                    //console.log(this);
                    var middleTracePos = this._positions[parseInt(this.positions.length / 2)];
                    clickposition=middleTracePos;//将一条轨迹的中间的那个点赋值给clickPoint
                    //将选中的某条轨迹的信息显示在balloon中，此函数与点击时显示balloon的那个函数一样
                    var cart2 = Cesium.SceneTransforms.wgs84ToWindowCoordinates(viewer.scene, middleTracePos);
                    showCrossPolylineInfo(this, cart2);
...
function getPosJDArrayFromBeginEndTime(satrec) {
    ...
    for (seconds = 0; seconds < secondsDifference; seconds += 1) {
        ...
        position = cart3ToSurface(position); //临时加的，将太空中的实际的点转换为地表上的点
...
/**
 *给定一个空中的cartesian3点，将其转化为地表面上的cartesian3点
 */
function cart3ToSurface(pos) {
    var cartographic, lon, lat, surfacePos;
    cartographic = viewer.scene.globe.ellipsoid.cartesianToCartographic(pos);

    lon = Cesium.Math.toDegrees(cartographic.longitude).toFixed(3);
    lat = Cesium.Math.toDegrees(cartographic.latitude).toFixed(3);
    surfacePos = Cesium.Cartesian3.fromDegrees(lon, lat);
